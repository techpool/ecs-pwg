...
const PwgUtil = function() {
	...
	this._allocateBucketToUser = async (accessToken, host) => {

		const bucketStats = await dataAccessor.getBucketStats(host, hostConfig[host].BUCKET.TOTAL);
		...
		return await dataAccessor.createOrUpdate(accessToken, host, bucketId, hostConfig[host].TTL_DAYS);

	};


	// Public functions
	this.getBucket = async(accessToken, host) => {
		...
		*/
		...
		const bucket = await dataAccessor.get(accessToken, host);
		...
		if (!isBucketValid) {
			...
			await dataAccessor.delete(bucket.accessToken, bucket.host, bucket.bucketId).catch(() => console.error(`ERROR: DELETE_EXPIRED :: ${accessToken} :: ${host}`));
	...
	this.deleteBucketAllocation = async(key) => {
		const bucket = await dataAccessor.getByKey(key).then(bucket => bucket ? JSON.parse(bucket) : null);
		if (bucket)
			return await dataAccessor.delete(bucket.accessToken, bucket.host, bucket.bucketId);
		return null;
	};
